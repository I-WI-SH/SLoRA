...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        # filename = './layers_ipt_list.csv'

        # # Write the list to a CSV file
        # with open(filename, 'w', newline='') as csvfile:
        #     csvwriter = csv.writer(csvfile)
        #     # Write the header (optional)
        #     header = [f'Layer {i+1}' for i in range(12)]
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    # use rankallocator choose different layers to train
                    if self.rankallocator is not None:
                        curr_rank, mask_threshold, layers_ipt_list, layers_fire_rate_list = self.rankallocator.update_and_mask(self.model, self.state.global_step)

                        """
                        our methods:
                        """
                        if layers_ipt_list is not None:
                            ...
                            combined_ipt_list = alpha * min_max_normalize(layers_ipt_list)  + beta * min_max_normalize(layers_fire_rate_list)
                            ...
                            for name, param in model.named_parameters(): 
                                ...
                                if 'lora' in name:
                                    ...
                                    if layer in combined_ipt_layers or 'lora_E' in name:
                            ...
                            if self.tb_writter is not None and self.state.global_step % self.model_args.tb_writter_loginterval == 0:
                                
                                self.tb_writter.add_scalars("combined_ipt_layers",
                                                            {f'top{i+1}': value for i,value in enumerate(combined_ipt_layers)}, self.state.global_step)
                                logger.info(f"Budget/train_sum_param {traing_sum_param}")
                                self.tb_writter.add_scalar("Budget/train_sum_param", traing_sum_param, self.state.global_step)
