...
from loralib import SLoRARankAllocator 
try:
...
class ModelArguments:
    ...
    apply_slora: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to apply SLoRA or not."},
    )
...
def main():
    ...
    # overwrite_output_dir is true so do not use last checkpoint
    ...
    # Setup logging
    logging.basicConfig(
        filename= os.path.join(training_args.root_output_dir, 'log.log'), filemode='a',
    ...
    if model_args.lora_type == "svd" and model_args.apply_slora:
        rankallocator = SLoRARankAllocator(
            model, 
            lora_r=model_args.lora_r,
